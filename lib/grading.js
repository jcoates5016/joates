...
import { MAX_CALIBRATION_SAMPLE, fitPlattScaling, fitPlattScalingByTier, CONFIDENCE_TIERS } from "./calibration.js";
...
export function foldIntoLedger(ledger, gradedPicks) {
  ...
  // Same {modelProb, hit} sample as recentForCalibration above, just split out per confidence tier too — see
  // lib/calibration.js's fitPlattScalingByTier for why a single pooled fit isn't enough (different tiers have
  // been miscalibrated by different amounts). Each tier's own array is capped independently at
  // MAX_CALIBRATION_SAMPLE, same reasoning as the pooled array: a rolling window of recent results per tier, not
  // an ever-growing history.
  ledger.recentForCalibrationByTier ||= { high: [], medium: [], low: [] };
  for (const pick of gradedPicks) {
    ...
    const sample = { modelProb: pick.rawModelProb ?? pick.modelProb, hit: !!pick.hit };
    ledger.recentForCalibration.push(sample);
    if (CONFIDENCE_TIERS.includes(pick.confidence)) {
      ledger.recentForCalibrationByTier[pick.confidence] ||= [];
      ledger.recentForCalibrationByTier[pick.confidence].push(sample);
    }
  }
  ...
  for (const tier of CONFIDENCE_TIERS) {
    const arr = ledger.recentForCalibrationByTier[tier];
    if (arr && arr.length > MAX_CALIBRATION_SAMPLE) ledger.recentForCalibrationByTier[tier] = arr.slice(-MAX_CALIBRATION_SAMPLE);
  }
...
export function summarizeLedger(ledger) {
  if (!ledger || !ledger.totals?.attempts) {
    return {
      hasData: false, totals: finalizeBucket(null), byConfidence: {}, byEdgeBucket: {},
      calibration: fitPlattScaling([]), calibrationByTier: fitPlattScalingByTier({})
    };
  }
  const byConfidence = {};
  ...
  return {
    ...
    // Per-tier fits, purely for display/transparency here — lib/pipeline.js independently fits and applies these
    // against the `all` ledger (the broadest real sample) before anything downstream reads modelProb; this is
    // what lets the dashboard show what's actually happening without re-deriving it a second, possibly
    // inconsistent way.
    calibrationByTier: fitPlattScalingByTier(ledger.recentForCalibrationByTier || {})
  };
}
