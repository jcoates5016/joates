...
// Same quality bar the Edge Board/Mispriced Bets tab already enforces (lib/pipeline.js's MIN_TRUE_EDGE) — a Top
// Pick is worthless as a "quick view of the sharpest plays" if it's allowed to include a real edge too thin to
// be more than market noise. Kept as its own constant (not imported) since pipeline.js doesn't export its copy;
// same value, same reasoning, see pipeline.js's own comment on it.
...
function categoryPicks(propRows, propTypeKey, limit) {
  ...
  return propRows
    .filter(r => r.propType === propTypeKey && r.model?.available && r.trueEdge > MIN_TRUE_EDGE &&
