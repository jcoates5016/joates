...
// Medium confidence's own higher bar (10pt) — same reasoning as pipeline.js's copy of this function, kept in sync
// by hand since this file can't import back from pipeline.js (circular import). See pipeline.js's comment on
// MIN_TRUE_EDGE_MEDIUM for the real recommended-only data (36.2% overall, 40.3% high vs 22.6% medium) behind it.
...
function categoryPicks(propRows, propTypeKey, limit) {
  ...
  return propRows
    .filter(r => r.propType === propTypeKey && r.model?.available && r.trueEdge > minEdgeFor(r.propType, r.confidence) &&
