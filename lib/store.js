...
// parlays: {} }, each an object of { [key]: { hash, result, updatedAt } } (keyed by oddID for props/lines/
// scouting, by a synthetic `_cacheKey` for parlays — see annotateParlaysWithAI in ai.js). `hash` is a content
// hash of exactly what got sent to Claude for that entry — when a refresh's entry hashes the same as last time,
// the cached `result` is reused instead of spending another Anthropic call, which is the whole point: coverage
// stayed at "every card," while the fix for the token cost that came with that is not re-asking a question whose
// real-world answer almost never changes twice in the same half hour.
export async function loadAiCache(season, week) {
  ...
  return cache && typeof cache === "object" ? cache : { props: {}, lines: {}, scouting: {}, parlays: {} };
}
...
// Anthropic daily spend governor (see lib/ai.js's estimateCostUsd/createSpendGuard and the "Anthropic cost
// controls" section of the README). One record, keyed by the current UTC calendar day — `date` rolls over
// automatically the first time a refresh runs after midnight UTC, archiving the finished day into a short
// `history` list (capped at 30 days) so the frontend can show a real spend trend, not just today's number.
const SPEND_KEY = "anthropic-spend-ledger.json";
export async function loadSpendLedger() {
  const ledger = await store("apex-edge-history").get(SPEND_KEY, { type: "json" }).catch(() => null);
  return ledger && typeof ledger === "object" ? ledger : { date: null, spentUsd: 0, callCount: 0, history: [] };
}
export async function saveSpendLedger(ledger) {
  await store("apex-edge-history").setJSON(SPEND_KEY, ledger);
}
