...
export function estimatePropProbability(factors, marketProb, coeffs = MODEL_COEFFS) {
  ...
  // Market steam — now a real cross-book read (lib/factors/market.js's computeSharpMoneySignal) instead of one
  // representative book's own open-vs-current move. Previously this only ever looked at f.marketMovement (this
  // week's rolling price-history series for whichever single book happened to have the best price) and only ever
  // fired in one direction (a price shortening), silently ignoring a price lengthening even though that's just as
  // real a market signal in the other direction. f.sharpMoney folds in every tracked book that has real open/
  // current data, requires at least 3 of them before saying anything (see MIN_BOOKS_FOR_SIGNAL), and is signed:
  // a real consensus move AWAY from this side now correctly nudges the estimate down, not just up. Same "no
  // historical multi-book odds-movement archive to backtest against" honesty caveat this nudge has always carried
  // — hand-set, unvalidated at the coefficient level, but now `firedFactors`-tracked under this same steam_move
  // key so scripts/refit-live-ledger.js will start checking whether the richer version actually beats the market
  // once enough real graded picks carry it.
  if (f.sharpMoney?.available && f.sharpMoney.direction !== 0) {
    const weight = (coeffs.steam_move || 0) * f.sharpMoney.direction * f.sharpMoney.sharpScore;
    const label = `real cross-book line movement — ${f.sharpMoney.label}`;
    nudgeSum += weight;
    contributors.push(label);
