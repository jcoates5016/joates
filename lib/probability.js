...
const RUN_PROPS = new Set(["rush_yds", "td_rush"]);
const PASS_PROPS = new Set(["pass_yds", "td_pass", "rec_yds", "receptions", "td_rec"]);
...
export function estimatePropProbability(factors, marketProb, coeffs = MODEL_COEFFS) {
  ...
  if (f.marketMovement?.available && f.marketMovement.priceMove < 0) {
    ...
    x += (coeffs.steam_move || 0) * magnitude;
    contributors.push(`market steaming this way (${f.marketMovement.priceMove} pts)`);
  }
