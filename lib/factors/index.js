...
import { computeLineMovementSeries, computeSharpMoneySignal } from "./market.js";
import { computeRefereeFactor } from "./referee.js";
import { computeNgsPassing, computeNgsRushing, computeNgsReceiving, buildNgsIndex } from "./nextgenstats.js";
import { computePressureFactor } from "./pressure.js";
import { computeQbrTrend, buildQbrIndex } from "./qbr.js";
...
export function createFactorEngine({
  ...
  // Empty maps (never null) when ngsData wasn't fetched (demo mode, or a failed fetch already logged non-fatally
  // by fetchNextGenStats) — every computeNgs* function already handles an empty/missing index by returning
  // `{ available: false }`, so this just keeps that the ONLY branch to reason about.
  const ngsPassingIndex = buildNgsIndex(ngsData?.passing);
  const ngsRushingIndex = buildNgsIndex(ngsData?.rushing);
  const ngsReceivingIndex = buildNgsIndex(ngsData?.receiving);
  // Same empty-array-is-fine treatment for real ESPN QBR (lib/factors/qbr.js) — computeQbrTrend already returns
  // unavailable on a missing/thin index, so demo mode and a failed live fetch both just fall through cleanly.
  const qbrIndex = buildQbrIndex(qbrRows);
  ...
  function assemblePropFactors(row) {
    ...
    return {
      ...
      // Real cross-book price movement (lib/factors/market.js) — every tracked book's own open-vs-current price,
      // not just the best-priced book's. This is what now actually feeds the steam_move nudge in
      // lib/probability.js (see that file's own comment) and what lib/sharpSignals.js ranks for the Sharp
      // Signals tab; marketMovement above stays as-is since computeMispricedScore already reads it.
      sharpMoney: computeSharpMoneySignal(row.allBookMovement, row.oddID, priceHistory, row.kickoff),
      // Real cross-book outlier detection (lib/analyze.js's computeBestAcrossBooks) — a price that beats the
      // panel by more than SUSPECT_EDGE_THRESHOLD AND is corroborated by the rest of the panel already agreeing
      // with the consensus (not a shared data error) gets flagged `staleValue` on the row itself: a majority of
      // the tracked books have already moved to the real price, one specific book just hasn't caught up yet. This
      // used to be surfaced only as a cosmetic badge on the Mispriced Bets tab — a real, corroborated signal that
      // never actually influenced `modelProb`, the number every ranking/parlay-tier decision is actually made on.
      // Wired in here the same way steam (marketMovement, above) already is, so it now genuinely moves the
      // estimate instead of just decorating it. See lib/probability.js's stale_line_value nudge for the math.
      staleLineValue: row.staleValue ? { available: true, edge: row.bestEdge, book: row.bestBook } : { available: false },
      // Non-bettable context read, same treatment as computeGameScript above — see lib/factors/referee.js for
      // why this only ever fires once a real assignment shows up in the schedule.
      referee: computeRefereeFactor(gameRow?.referee, schedule),
      // Next Gen Stats player efficiency — real, tracking-data-derived numbers isolating this player's own skill
      // (see lib/factors/nextgenstats.js). Only one of the three ever actually returns available:true for a
      // given player (gated by position inside each function), but all three are cheap to call and it keeps the
      // "one factor, one key" shape every other factor here already follows instead of a single overloaded key.
      ngsPassing: computeNgsPassing(player, ngsPassingIndex),
      ngsRushing: computeNgsRushing(player, ngsRushingIndex),
      ngsReceiving: computeNgsReceiving(player, ngsReceivingIndex),
      // Real ESPN Total QBR trend (lib/factors/qbr.js) — QB position only (computeQbrTrend gates on this
      // itself; the check here just avoids the wasted map lookup for every non-QB row).
      qbr: player.position === "QB" ? computeQbrTrend(player, qbrIndex) : { available: false },
      // Pass-protection/pressure matchup (lib/factors/pressure.js) — only meaningful for the passing game, the
      // same "only compute it where it plausibly applies" rule secondaryInjury/frontSevenInjury already follow.
      pressure: ["pass_yds", "td_pass", "rec_yds", "receptions", "td_rec"].includes(row.propType)
        ? computePressureFactor(player.team, row.opponent, teamSeasonIndex) : { available: false },
