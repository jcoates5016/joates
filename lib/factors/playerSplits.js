...
// Fixes the trade-mixing bug logged in README's "Known limitations" section: `computeFormFactor`,
// `computeUsageFactor`, and `computeTeammateOutTendency` used to pull a player's trailing game log keyed only by
// his name, with no filter for which team he was actually on for each game — so a midseason trade silently
// blended pre-trade and post-trade context into one trailing average (DJ Moore's "last 10 games" the week after
// a trade would mix Panthers/Bears-era usage with a completely different scheme, target competition, and QB).
// `computeTeammateOutTendency` was the worst case: a traded player's OLD team's games all got bucketed as "games
// without [new teammate]," since that teammate was never on the old roster — not a noisy signal, a structurally
// wrong one, since that comparison never actually happened on the field.
//
// Fix direction taken straight from the README's own logged proposal: filter the trailing window to games where
// the row's own `recent_team`/`team` matches the player's CURRENT `player.team` first, falling back to the full
// cross-team history only when the current-team-only sample is too thin (under MIN_CURRENT_TEAM_GAMES) to say
// anything — a player who's been on one roster his whole career is unaffected either way, since every row
// already matches. `computeVenueSplit`/`computeWeatherSplitHistorical`/`computeBirthdaySplit` stay as-is per the
// same README note: they measure the player's own physical tendencies more than his team's scheme, so mixing
// eras matters less for those.
const MIN_CURRENT_TEAM_GAMES = 3;
function currentTeamRows(rows, player) {
  if (!player?.team || !rows.length) return rows;
  const filtered = rows.filter(r => normTeam(r.recent_team || r.team) === player.team);
  return filtered.length >= MIN_CURRENT_TEAM_GAMES ? filtered : rows;
}
...
// Gates the whole factor on the teammate's CURRENT, this-refresh injury status — not just historical game-log
// absence. Before this, `injuriesByTeam` was accepted as a parameter and never actually read: "without teammate"
// was decided purely from whether the teammate's game log had a row that week at all (a bye, a benching, a trade,
// an old injury — any absence counted), with zero connection to whether he's actually playing THIS week. That
// produced real, confusing live output — a writeup reading "Without Omarion Hampton on the field..." for a game
// where Hampton was active and expected to play, because some OTHER week in the log happened to be missing him.
// Now the factor is skipped entirely unless the teammate is presently listed Out, Doubtful, or Questionable (per
// Jon's call: Questionable counts as "not fully expected to play," matching how computeOLineInjuryFlag already
// treats current-status gating everywhere else in this app) — a real, checkable, current fact, or nothing.
function isTeammateCurrentlyLimited(status) {
  return /out|doubtful|questionable/i.test(status || "");
}
...
export function computeTeammateOutTendency(player, teammateName, injuriesByTeam, gameLogIndex, propType) {
  ...
  const teamList = (injuriesByTeam || {})[player.team] || [];
  const teammateEntry = teamList.find(x => (x.name || "").toLowerCase() === teammateName.toLowerCase());
  if (!isTeammateCurrentlyLimited(teammateEntry?.status)) return { available: false };
  const rows = currentTeamRows(gameLogIndex.get(player._logKey) || [], player);
  ...
  return {
    available: true, statLabel: statLabelFor(propType), teammateName, teammateStatus: teammateEntry.status,
    withAvg: avg(withTeammate), withN: withTeammate.length, withoutAvg: avg(withoutTeammate), withoutN: withoutTeammate.length
  };
}
...
export function computeUsageFactor(player, gameLogIndex, snapsByKey) {
  const rows = currentTeamRows(gameLogIndex.get(player._logKey) || [], player).slice(-3);
