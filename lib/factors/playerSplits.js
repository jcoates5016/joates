...
// The stat this prop type is actually graded on, per game row — NOT yet compared to a threshold.
function statValueFn(propType) {
  return {
...
function statLabelFor(propType) {
  return {
...
// Anytime-TD-style props ("did this happen at all") have no numeric market line to compare against — "at
// least 1" is the whole bet, so the threshold is 0. A real yardage or reception prop always has the actual
// line from the odds feed and must be graded against THAT number, not a hardcoded stand-in.
function thresholdFor(propType, line) {
  if (["td", "td_pass", "td_rush", "td_rec"].includes(propType)) return 0;
