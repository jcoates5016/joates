...
// Real historical weekly injury reports — verified live the same way as every other URL in this file (a direct
// download, not assumed from memory): HTTP 200 with real CSV content for 2023/2024/2025, columns confirmed as
// `season, game_type, team, week, gsis_id, position, full_name, first_name, last_name, report_primary_injury,
// report_secondary_injury, report_status, practice_primary_injury, practice_secondary_injury, practice_status,
// date_modified`. `report_status` (Out/Doubtful/Questionable/blank) is the official weekly designation — the
// same status shape the live app's own `injuriesByTeam[team][].status` carries — which is what finally makes
// scripts/backtest.js able to test tendency_usage_bump/secondary_injury/oline_injury_penalty/front_seven_injury
// for real, instead of leaving them hand-set forever (see that script's own comments on each).
const INJURIES_RELEASE = (season) => `https://github.com/nflverse/nflverse-data/releases/download/injuries/injuries_${season}.csv`;
...
// REG-season only (game_type filter mirrors every other season-scoped fetch in this file) — postseason injury
// rows exist in the same file but backtest.js's walk-forward loop never walks postseason weeks anyway.
export async function fetchInjuryHistory(season, log = () => {}) {
  try {
    const rows = await fetchCSV(INJURIES_RELEASE(season));
    const reg = rows.filter(r => !r.game_type || r.game_type === "REG");
    log(`Loaded ${reg.length} historical injury-report rows for ${season} (${rows.length} total incl. postseason).`);
    return reg;
  } catch (e) { log(`Injury-history fetch failed for ${season}: ${e.message}`); return []; }
}
