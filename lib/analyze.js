...
// enough to prove anything. This is a curated 8, not all 77: object cost on this API scales with events ×
// markets in a way the docs don't fully specify, and there's no confirmation it's free to add more bookmakerIDs
// to the same call — see the odds budget note in README before adding more. `label` is what shows in the AI
// prompt payload and the parlay write-up text; `short` is the pill badge on every card.
export const BOOKS = {
  ...
  fanatics: { label: "Fanatics", short: "FAN" },
...
// couple points. Past this threshold, a mismatched side/price somewhere in the pipeline is far more likely
// than a real -230-quality price sitting at +180. Flag it instead of trusting it blindly: `suspect` rows are
// excluded from Mispriced Bets, AI commentary and parlay legs, and get a visible warning in the UI. Worth
// watching now that BOOKS covers 8 books instead of 2: a book that's just genuinely slow to move a line (real,
// bettable value) can look identical to a data error at this threshold, and this can't tell the two apart on
// its own — the raw fields stay visible on every flagged row specifically so you can eyeball which one it is.
// Raise or remove this once the feed's exact field shape has been confirmed against enough live runs.
