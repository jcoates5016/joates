...
// to work with. Each has a Total Over plus three heavily-favored Overs props (~60%+ implied probability, so
// they clear parlays.js's MIN_LEG_PROBABILITY floor via the book-price fallback even though no game-log history
// exists for these players to produce a real modelProb) on two new, roster-only players. Kickoffs are fixed,
// real calendar timestamps — not "N days from now" — specifically so classifyKickoffWindow's Sunday 1:00pm/
// 4:00pm ET bucketing can be tested deterministically; Nov 1, 2026 is a real Sunday, safely past that year's
// November daylight-saving change, so the Eastern-time conversion itself is actually exercised.
function fillerGame({ eventID, home, away, kickoffISO, wrName, rbName }) {
  return {
    eventID,
    teams: {
    ...
    odds: {
      tot_over: { betTypeID: "ou", periodID: "game", statID: "total_points", sideID: "over", bookOverUnder: 44.5, fairOverUnder: 44.5, oddID: `${eventID}_tot_over`, fairOdds: -110, byBookmaker: { draftkings: { odds: -110 }, fanduel: { odds: -108 } } },
      wr_rec_yds: { betTypeID: "ou", periodID: "game", statID: "receiving_yards", statEntityID: `${eventID}_wr`, playerName: wrName, sideID: "over", bookOverUnder: 49.5, fairOverUnder: 49.5, oddID: `${eventID}_wr_rec_yds_over`, fairOdds: -170, byBookmaker: { draftkings: { odds: -170 }, fanduel: { odds: -165 }, espnbet: { odds: -175 } } },
      wr_receptions: { betTypeID: "ou", periodID: "game", statID: "receptions", statEntityID: `${eventID}_wr`, playerName: wrName, sideID: "over", bookOverUnder: 3.5, fairOverUnder: 3.5, oddID: `${eventID}_wr_receptions_over`, fairOdds: -180, byBookmaker: { draftkings: { odds: -180 }, fanduel: { odds: -175 }, espnbet: { odds: -185 } } },
      rb_rush_yds: { betTypeID: "ou", periodID: "game", statID: "rushing_yards", statEntityID: `${eventID}_rb`, playerName: rbName, sideID: "over", bookOverUnder: 39.5, fairOverUnder: 39.5, oddID: `${eventID}_rb_rush_yds_over`, fairOdds: -175, byBookmaker: { draftkings: { odds: -175 }, fanduel: { odds: -170 }, espnbet: { odds: -180 } } }
    }
  };
}
...
export function buildDemoData(currentSeason, historySeasons) {
  ...
  // Second Sunday-1:00pm-ET game — pairs with demo-1 for the 1:00 slate parlay test (2 games in that window).
  fillerGame({ eventID: "demo-2", home: "NYJ", away: "MIA", kickoffISO: "2026-11-01T18:00:00Z", wrName: "Demo Marlin", rbName: "Demo Gotham" }),
  // Two Sunday-4:05/4:25pm-ET games — pair with each other for the 4:00 slate parlay test.
  fillerGame({ eventID: "demo-3", home: "PHI", away: "DAL", kickoffISO: "2026-11-01T21:05:00Z", wrName: "Demo Ranger", rbName: "Demo Liberty" }),
  fillerGame({ eventID: "demo-4", home: "LAR", away: "SEA", kickoffISO: "2026-11-01T21:25:00Z", wrName: "Demo Sound", rbName: "Demo Angeleno" }),
  // A Thursday-night game — outside both Sunday slate windows on purpose, so it only ever shows up via its own
  // Same Game Parlay, never pooled into a slate build.
  fillerGame({ eventID: "demo-5", home: "HOU", away: "TEN", kickoffISO: "2026-10-30T00:15:00Z", wrName: "Demo Volunteer", rbName: "Demo Astro" })
  ];
  ...
  const rosterRows = [
    ...
    // just anchors to the market (see estimatePropProbability) when no real evidence is available.
