...
export function buildDemoData(currentSeason, historySeasons) {
  ...
  // giving BOTH would push whichever band its fair price lands in (necessarily one of Low/Medium/High, since
  // Nuke's own floor is the same 55% floor those three partition) from 2 legs to 4, which happens to equal
  // Medium/High's own leg count and would make this "always-too-thin" fixture accidentally succeed on its own.
  // rb2's receptions leg instead stays a second, ordinary Medium-band leg — real Mega-payout material (its own
  // price still stacks into Mega's combined target below) without double-counting into a band this fixture is
  // supposed to always fall short of.
  (() => {
    const g = fillerGame({ eventID: "demo-5", home: "HOU", away: "TEN", kickoffISO: "2026-10-30T00:15:00Z", wr1Name: "Demo Volunteer", rb1Name: "Demo Astro", wr2Name: "Demo Volunteer Two", rb2Name: "Demo Astro Two",
      wrRecYdsOdds: -370, wrRecOdds: -185, rbRushOdds: -140, rbRecFairOdds: -140, rbRecDkOdds: 130 });
