...
// legs into each of parlays.js's four fixed probability bands instead of leaving band coverage to chance.
...
// to work with, AND to deliberately spread real legs across all four of parlays.js's fixed probability bands
// (Low >=75%, Medium 65-75%, High 60-65%, Mega 55-60%) on FOUR new, roster-only players per game (two WR/RB
// pairs, both pairs priced identically per band) — two legs per band per game. No single game needs to fill
// every band on its own (a real NFL slate rarely offers legs spanning all four bands in one game either — see
// the design note on RISK_TIERS in parlays.js), but two of these games pooled into the same Sunday slate window
// give that window 4 real legs per band — exactly enough to fill every tier (Low needs 3, Medium/High/Mega need
// 4) while still leaving each INDIVIDUAL game's own Same Game Parlay honestly short (only 2 real legs per band)
// — a deliberate, checkable demonstration of why this tier structure is pool-size-dependent. Kickoffs are fixed,
...
function fillerGame({ eventID, home, away, kickoffISO, wr1Name, rb1Name, wr2Name, rb2Name, wrRecYdsOdds, wrRecOdds, rbRushOdds, rbRecOdds }) {
  return {
    eventID,
    teams: {
    ...
    odds: {
      ...
      rb1_receptions: fillerProp(eventID, "rb1_receptions", "receptions", `${eventID}_rb1`, rb1Name, 1.5, rbRecOdds),
      ...
      rb2_receptions: fillerProp(eventID, "rb2_receptions", "receptions", `${eventID}_rb2`, rb2Name, 1.5, rbRecOdds)
    }
  };
}
...
export function buildDemoData(currentSeason, historySeasons) {
  ...
  fillerGame({ eventID: "demo-1b", home: "NYG", away: "WAS", kickoffISO: "2026-11-01T18:00:00Z", wr1Name: "Demo Giant", rb1Name: "Demo Commander", wr2Name: "Demo Giant Two", rb2Name: "Demo Commander Two",
    wrRecYdsOdds: -410, wrRecOdds: -205, rbRushOdds: -158, rbRecOdds: -132 }),
  ...
  fillerGame({ eventID: "demo-2", home: "NYJ", away: "MIA", kickoffISO: "2026-11-01T18:00:00Z", wr1Name: "Demo Marlin", rb1Name: "Demo Gotham", wr2Name: "Demo Marlin Two", rb2Name: "Demo Gotham Two",
    wrRecYdsOdds: -400, wrRecOdds: -200, rbRushOdds: -155, rbRecOdds: -130 }),
  ...
  fillerGame({ eventID: "demo-3", home: "PHI", away: "DAL", kickoffISO: "2026-11-01T21:05:00Z", wr1Name: "Demo Ranger", rb1Name: "Demo Liberty", wr2Name: "Demo Ranger Two", rb2Name: "Demo Liberty Two",
    wrRecYdsOdds: -195, wrRecOdds: -160, rbRushOdds: -135, rbRecOdds: -410 }),
  fillerGame({ eventID: "demo-4", home: "LAR", away: "SEA", kickoffISO: "2026-11-01T21:25:00Z", wr1Name: "Demo Sound", rb1Name: "Demo Angeleno", wr2Name: "Demo Sound Two", rb2Name: "Demo Angeleno Two",
    wrRecYdsOdds: -420, wrRecOdds: -128, rbRushOdds: -205, rbRecOdds: -158 }),
  // A Thursday-night game — outside both Sunday slate windows on purpose, so it only ever shows up via its own
  // Same Game Parlay (honestly falling short of every tier, since 2 legs/band never clears a 3-4-leg tier on its
  // own), never pooled into a slate build.
  fillerGame({ eventID: "demo-5", home: "HOU", away: "TEN", kickoffISO: "2026-10-30T00:15:00Z", wr1Name: "Demo Volunteer", rb1Name: "Demo Astro", wr2Name: "Demo Volunteer Two", rb2Name: "Demo Astro Two",
    wrRecYdsOdds: -160, wrRecOdds: -132, rbRushOdds: -390, rbRecOdds: -198 })
  ];
