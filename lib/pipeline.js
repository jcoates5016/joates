...
import { fitPlattScaling, applyPlattScaling, fitPlattScalingByTier, applyTieredPlattScaling } from "./calibration.js";
...
import { buildSharpSignals } from "./sharpSignals.js";
...
// the frontend's Mispriced Bets tab uses (see the `mispriced` filter further down), just without its top-20
// display cap (that cap is a UI convenience, not the real definition of "this was a flagged edge").
...
// Anytime TD gets its own, much higher bar — real diagnostic data (scripts/diagnose-accuracy.js, run against the
// live results ledger after the live-line cleanup) showed Anytime TD hitting at 13.5% against the model's own
// already-modest 18.8% average confidence on it, out of 259 real graded picks — by far the single worst-performing
// prop type, and the majority of everything the app has ever evaluated. Touchdowns are a bursty, low-frequency
// event: a player's last-10-game TD rate is mostly noise, and the model reads a couple of recent scores as a real
// trend it isn't. Rather than exclude the market outright (Jon's call, discussed directly), it stays eligible
// everywhere but needs 5x the real edge before it counts as a genuine recommendation — see minEdgeFor below.
const MIN_TRUE_EDGE_TD = 0.15;
// Medium confidence gets its own higher bar too, for the same reason TD did — real data, not intuition. Once the
// wasEdgeBoard backfill (scripts/backfill-wasedgeboard.js) recovered the ~95% of history that a merge bug had been
// silently dropping, the honest recommended-only track record came out to 229 real picks, 36.2% hit — and split by
// tier, high confidence hit 40.3% (176 picks) while medium hit only 22.6% (53 picks), the ordering you'd expect but
// with medium dragging the blended number down hard. `confidenceTier` (lib/probability.js) is a SAMPLE-SIZE bucket,
// not a probability bucket — "medium" just means less data behind the estimate, not a lower predicted probability —
// so raising its edge bar is the same fix as TD's: don't exclude it, just require a bigger apparent mispricing
// before a thinner-evidence pick counts as a real recommendation. 0.10 (10pt) is a reasoned starting point — roughly
// 3x the 3pt baseline, deliberately less extreme than TD's 5x since medium-confidence props aren't the single
// worst-performing category the way Anytime TD was — not a backtested value (there's no historical wasEdgeBoard
// ledger to re-test against; this whole exercise IS what created that ledger). Revisit once enough real
// medium-confidence recommendations accumulate under this new bar to judge whether 10pt is the right level.
const MIN_TRUE_EDGE_MEDIUM = 0.10;
// Single source of truth for "how much edge does this specific prop type/confidence combo need to count as a real
// recommendation" — used everywhere MIN_TRUE_EDGE used to be checked directly (wasEdgeBoard below, the `mispriced`
// filter, and the trueEdgeProps count further down). lib/topPicks.js keeps its own copy of this same logic (it's
// imported BY this file, so it can't import back from here) — keep both in sync if this ever changes. TD's bar
// takes priority over the confidence-based one (a medium-confidence TD still only needs TD's 15pt bar, not 15+10);
// everything else falls through to the confidence-based bar.
function minEdgeFor(propType, confidence) {
  if (propType === "td") return MIN_TRUE_EDGE_TD;
  if (confidence === "medium") return MIN_TRUE_EDGE_MEDIUM;
  return MIN_TRUE_EDGE;
}

// Every event without a resolvable kickoff is dropped too, not kept — "can't confirm this is still pregame"
// fails the same way "confirmed already started" does; this app would rather show fewer props than a wrong
// one. Exported and pure (takes `now` as a parameter) so scripts/dry-run.js can unit-test it directly against
// synthetic events — the real call site (runPipeline below) only ever runs live, gated `!demo`, same reasoning
// as the CLV/grading/parlay-tracking logic elsewhere in this file.
export function filterPregameEvents(events, now = new Date()) {
  return events.filter(evt => {
    const kickoff = evt.status?.startsAt || evt.scheduled || evt.startTime || null;
    const kickoffTime = kickoff ? new Date(kickoff).getTime() : NaN;
    return !isNaN(kickoffTime) && kickoffTime > now.getTime();
  });
}
...
function buildGradablePicks(propRows, season, week) {
  return propRows
    ...
    .map(r => ({
      ...
      // Which of lib/probability.js's real nudges actually fired for THIS pick, captured once at the same moment
      // as pickPrice/pickBook (never overwritten on a later refresh — see the merge loop's preservation logic in
      // runPipeline) — just the coefficient keys (e.g. "redzone_share", "form_hot"), not the full label/weight,
      // to keep each saved pick small; the weight each key carried is always in lib/modelCoeffs.js's
      // MODEL_COEFFS, so a later analysis can always join back against whatever weight was active then. Before
      // this field existed, the results ledger recorded WHETHER a pick hit but never WHICH real factors were
      // behind its number — meaning no analysis, no matter how much real graded history piled up, could ever
      // answer "do picks where redzone_share fired actually hit more than ones where it didn't." This is what
      // makes that question answerable, going forward, off real live results instead of scripts/backtest.js's
      // "beat your own trailing average" proxy.
      firedFactors: (r.modelContributorDetails || []).map(d => d.key),
      // effectiveN/nudgeCapped/rawNudgeSum (lib/probability.js) — never saved before this. Captured once here,
      // same "never overwritten on a later refresh" treatment as pickPrice/pickBook, so a future analysis (once
      // enough real graded volume exists) can check whether confidenceTier's effectiveN>=8 cutoff for "high"
      // actually lines up with where real accuracy improves, and whether picks the nudge cap actually clipped
      // hit at a different rate than ones it didn't — both currently asserted, neither yet checked against real
      // outcomes.
      effectiveN: r.effectiveN ?? null, nudgeCapped: !!r.nudgeCapped, rawNudgeSum: r.rawNudgeSum ?? null,
      ...
      wasEdgeBoard: r.trueEdge > minEdgeFor(r.propType, r.confidence) && ["medium", "high"].includes(r.confidence) && !r.teamMismatch && !r.suspect,
      // Wall-clock timestamp of the refresh that first saved this pick — captured once, preserved forever by the
      // merge loop below, exactly like pickPrice/pickBook. Since this field only exists on picks saved AFTER the
      // filterPregameEvents fix (see that function and the "Pregame-only filtering" section of the README), its
      // mere presence is proof the pick was built from an event that had strictly not kicked off yet: propRows
      // only ever contains events that already passed filterPregameEvents by the time buildGradablePicks runs. A
      // pick with no capturedAt is an old, pre-fix pick whose pregame status can't be trusted directly — that's
      // exactly the case scripts/clean-live-line-contamination.js's price-history heuristic exists to sort out.
      // Going forward, no heuristic is ever needed again: capturedAt being set IS the proof.
...
function buildGradableParlays(parlayAttempts, season, week) {
  return parlayAttempts.filter(p => p.ok).map(p => ({
    ...
    // Same proof-of-pregame-capture marker as buildGradablePicks's capturedAt above (see its comment) — every
    // leg here comes from allParlayAttempts, which is itself built from propRows, which by this point in
    // runPipeline has already been through filterPregameEvents. Preserved once-captured, never refreshed, by
...
export async function runPipeline(opts) {
  ...
  // Pregame-only, no exceptions: this app is a pregame sharp-line finder, not a live-betting tool, and
  // SportsGameOdds keeps returning odds for a game after it kicks off — that "line" is now a continuously
  // market-adjusted in-play number (reflecting whatever the player has already done in the game), not a real
  // pregame market. Treating it as one is a real, confirmed bug: a refresh run mid-game will otherwise (1) show
  // an in-progress game's live-drifted number on the Edge Board/Player Props/Parlays as if it were a fresh
  // pregame edge, and (2) — worse — permanently bake that live number into a pick's saved `line`/`pickPrice` the
  // first time that oddID gets captured for grading (buildGradablePicks below), corrupting the results ledger
  // for good, since a pick's line is captured once and never overwritten. A live line has nothing to do with
  // whether the model's PREGAME read on this player was sharp, so grading against it is comparing the wrong
  // thing and drags the whole track record down with noise, not a real signal. See filterPregameEvents (module
  // scope, above) for exactly what counts as pregame.
  if (!demo) {
    const beforeKickoffFilter = events.length;
    events = filterPregameEvents(events);
    if (events.length !== beforeKickoffFilter) {
      log(`Excluded ${beforeKickoffFilter - events.length} event(s) that have already kicked off (or have no resolvable kickoff time) — this app only ever shows/grades pregame lines, never a live/in-play one.`);
    }
  }
  ...
  for (const row of propRows) {
    ...
    if (model.available) {
      ...
      // effectiveN/nudgeCapped/rawNudgeSum weren't captured on saved picks before — meaning the confidence-tier
      // cutoffs (lib/probability.js's confidenceTier: effectiveN>=8 for "high") could never actually be checked
      // against real ledger outcomes, only asserted. Saving them here (buildGradablePicks below carries them onto
      // the persisted pick) is what will let a future analysis — once enough real graded volume exists — check
      // whether 8 games is really where reliability improves, instead of it being a number nobody's verified.
      row.effectiveN = model.effectiveN; row.nudgeCapped = model.nudgeCapped; row.rawNudgeSum = model.rawNudgeSum;
    }
  }
  ...
  if (!demo) {
    try {
      ...
      // Per-tier calibration (lib/calibration.js's fitPlattScalingByTier) replaces the single pooled fit that
      // used to apply the same A/B correction to every confidence tier at once. A real run showed "high
      // confidence" running ~20 points hot and "medium confidence" running ~21 points hot off a much lower base
      // rate — two different miscalibration shapes a single 2-parameter Platt fit can't both correct at once.
      // The pooled `globalCalibration` fit is kept as a fallback (applyTieredPlattScaling below reaches for it)
      // for any tier that hasn't individually cleared MIN_CALIBRATION_PICKS yet, so a thin tier still gets SOME
      // correction instead of none while its own sample builds up.
      const globalCalibration = fitPlattScaling(calibrationLedger.recentForCalibration || []);
      const tierCalibrations = fitPlattScalingByTier(calibrationLedger.recentForCalibrationByTier || {});
      let anyApplied = false;
      for (const row of propRows) {
        if (!row.model?.available) continue;
        const { calibratedProb, source } = applyTieredPlattScaling(row.modelProb, row.confidence, tierCalibrations, globalCalibration);
        if (source === "none") continue;
        anyApplied = true;
        row.rawModelProb = row.modelProb;
        row.modelProb = calibratedProb;
        row.trueEdge = +(calibratedProb - row.marketProb).toFixed(4);
        row._calibrationSource = source;
      }
      if (anyApplied) {
        const tierSummary = ["high", "medium", "low"].map(t => {
          const fit = tierCalibrations[t];
          return fit?.available ? `${t}: A=${fit.A.toFixed(3)} B=${fit.B.toFixed(3)} n=${fit.n}` : `${t}: not enough data yet (n=${fit?.n ?? 0})`;
        }).join(" · ");
        const globalSummary = globalCalibration.available
          ? `A=${globalCalibration.A.toFixed(3)} B=${globalCalibration.B.toFixed(3)} n=${globalCalibration.n}`
          : "not enough data yet";
        log(`Applied per-tier Platt-scaling recalibration — ${tierSummary}. Pooled fallback (used for any tier still below the per-tier floor): ${globalSummary}.`);
      }
  ...
  // Two separate track records, not one blended number — see the fold logic below for why this split exists.
  // `all` is every prop the app ever evaluated (huge majority never actually recommended, tracked only so the
  // Prop Bets tab has real history and Platt-scaling has a real sample to fit against). `recommended` is the
  // one that actually answers "is this app worth trusting" — only picks that cleared the real edge bar
  // (wasEdgeBoard) and were actually shown to Jon as a real bet. Conflating the two into one number (the
  // original design) made the model look far worse than it is: most of what's in `all` was never a bet the app
  // told anyone to make, just background props it glanced at and correctly declined to recommend.
  let trackRecord = { all: summarizeLedger({}), recommended: summarizeLedger({}) };
  ...
  if (!demo) {
    try {
      ...
      for (const p of freshPicks) {
        ...
        if (existing?.wasEdgeBoard != null) p.wasEdgeBoard = existing.wasEdgeBoard;
        // Preserve the ORIGINAL capture timestamp, not this refresh's — same reasoning as pickPrice/pickBook
        // above. Without this, capturedAt would silently update every refresh and stop meaning "when was this
        // first ever saved."
        if (existing?.capturedAt) p.capturedAt = existing.capturedAt;
        // Same "once captured, never overwritten" treatment — a later refresh's factor set (injury news, a
        // fresh weather forecast) isn't "what the model saw when this pick was actually flagged," the same
        // reason pickPrice/pickBook don't drift either.
        if (existing?.firedFactors) p.firedFactors = existing.firedFactors;
        // Same "once captured, never overwritten" treatment — effectiveN/nudgeCapped/rawNudgeSum describe what
        // the model saw the moment this pick was first flagged, not whatever a later refresh's factor set works
        // out to.
        if (existing?.effectiveN != null) p.effectiveN = existing.effectiveN;
        if (existing?.nudgeCapped != null) p.nudgeCapped = existing.nudgeCapped;
        if (existing?.rawNudgeSum != null) p.rawNudgeSum = existing.rawNudgeSum;
      ...
      if (newlyGraded.length) {
        ...
        // Same picks, folded a second time into a separate `recommended`-only bucket nested in the same ledger
        // object — this is what makes trackRecord.recommended answer "of what was actually recommended, what
        // hit," completely independent of the (much larger) `all` bucket above. Nested rather than a second
        // stored blob so this stays a single read/write against calibration-ledger.json.
        const recommendedGraded = newlyGraded.filter(p => p.wasEdgeBoard);
        if (recommendedGraded.length) {
          calibrationLedger.recommended ||= {};
          foldIntoLedger(calibrationLedger.recommended, recommendedGraded);
        }
        await saveCalibrationLedger(calibrationLedger);
        log(`Graded ${newlyGraded.length} completed pick(s) against real results (${recommendedGraded.length} of them actual Edge Board recommendations).`);
      }
      trackRecord = { all: summarizeLedger(calibrationLedger), recommended: summarizeLedger(calibrationLedger.recommended || {}) };
    ...
    try {
      ...
      for (const p of freshParlays) {
        ...
        if (existing) {
          ...
          // Preserve the original capture timestamp, same reasoning as the picks ledger's capturedAt above.
          if (existing.capturedAt) p.capturedAt = existing.capturedAt;
        }
        byKey.set(p.key, p);
      }
  ...
  // well-supported trend. minEdgeFor (module scope, above) is a real threshold in probability terms: below ~3
  // points of edge for most prop types (much higher for Anytime TD), the model isn't saying anything the market
  // doesn't already know.
  const mispriced = propRows.filter(r => r.model?.available && r.trueEdge > minEdgeFor(r.propType, r.confidence) &&
  ...
  // Sharp Signals: a market-wide scan for real, broad cross-book price movement — see lib/sharpSignals.js and
  // lib/factors/market.js's computeSharpMoneySignal for exactly what this can and can't detect (line movement
  // across the tracked book panel, not confirmed bet-percentage/handle data, which this app has no source for).
  const sharpSignals = buildSharpSignals(propRows);
  ...
  return {
    ...
    stats: {
      ...
      trueEdgeProps: propRows.filter(r => (r.trueEdge ?? 0) > minEdgeFor(r.propType, r.confidence) && ["medium", "high"].includes(r.confidence)).length,
    ...
    propRows, mispriced, topPicks, sharpSignals, injuryEscalations, parlays, sameGameParlays, slateParlays, trackRecord,
