...
import { annotateGameLinesWithAI, annotatePropsWithAI, annotateScoutingTakes, annotateParlaysWithAI, pruneAiCache, selectAiEligible, AI_NOTE_LIMIT } from "./ai.js";
...
export async function runPipeline(opts) {
  ...
  selectAiEligible(gameLines, propRows, AI_NOTE_LIMIT);
  ...
  const sameGameParlays = buildSameGameParlays(gameLines, propRows);
  const slateParlays = buildSlateParlays(gameLines, propRows);
  ...
  if ((aiOn || scoutOn) && anthropicApiKey && !demo) {
    let aiCache = { props: {}, lines: {}, scouting: {}, parlays: {} };
    ...
    aiCache.props ||= {}; aiCache.lines ||= {}; aiCache.scouting ||= {}; aiCache.parlays ||= {};
  ...
  return {
    ...
    gameLines, propRows, mispriced, parlays, sameGameParlays, slateParlays, trackRecord, logs
  };
}
