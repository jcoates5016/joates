...
// Single source of truth for "how much edge does this specific prop type need to count as a real recommendation"
// — used everywhere MIN_TRUE_EDGE used to be checked directly (wasEdgeBoard below, the `mispriced` filter, and
// the trueEdgeProps count further down). lib/topPicks.js keeps its own copy of this same logic (it's imported BY
// this file, so it can't import back from here) — keep both in sync if this ever changes.
...
function buildGradablePicks(propRows, season, week) {
  return propRows
    ...
    .map(r => ({
      ...
      wasEdgeBoard: r.trueEdge > minEdgeFor(r.propType) && ["medium", "high"].includes(r.confidence) && !r.teamMismatch && !r.suspect,
...
export async function runPipeline(opts) {
  ...
  const mispriced = propRows.filter(r => r.model?.available && r.trueEdge > minEdgeFor(r.propType) &&
  ...
  return {
    ...
    stats: {
      ...
      trueEdgeProps: propRows.filter(r => (r.trueEdge ?? 0) > minEdgeFor(r.propType) && ["medium", "high"].includes(r.confidence)).length,
