...
export async function runPipeline(opts) {
  ...
  // including selectAiEligible just below (which ranks props by real modelProb to decide which get an AI note) —
  // so every consumer of modelProb/trueEdge from this point on (AI selection, mispriced ranking, parlays,
  // topPicks, the frontend) sees the same, already-calibrated number rather than some seeing raw and others
  // calibrated depending on where in the pipeline they read it. `calibrationLedger` is loaded once here and
