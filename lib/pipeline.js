...
import { annotateGameLinesWithAI, annotatePropsWithAI, annotateScoutingTakes, annotateParlaysWithAI } from "./ai.js";
import { buildDemoData } from "./demoData.js";
import { STADIUMS } from "./stadiums.js";
import { normTeam } from "./teamCodes.js";
import { appendInjurySnapshot, loadInjuryHistory, appendPriceSnapshots, loadPriceHistory } from "./store.js";
...
export async function runPipeline(opts) {
  ...
  let events, statRows, schedRows, rosterRows, snapRows, pbpRows;
  if (demo) {
    ({ events, statRows, schedRows, rosterRows, snapRows, pbpRows } = buildDemoData(currentSeason, historySeasons));
  ...
  const injuriesByTeam = demo ? {} : await fetchInjuries(teams, log);
  ...
  if (aiOn && anthropicApiKey) {
    await Promise.all([
      annotateGameLinesWithAI(gameLines, anthropicApiKey, log),
      annotatePropsWithAI(propRows, anthropicApiKey, log)
    ]);
  }
  if (scoutOn && anthropicApiKey) await annotateScoutingTakes(propRows, anthropicApiKey, log);
