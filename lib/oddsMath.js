...
// Shared by lib/probability.js and scripts/backtest.js: converting a probability to/from log-odds is what lets
// several independent small signals (a matchup edge, an injury flag, a schedule penalty) combine by simple
// addition instead of each fighting over how to stack multiplicatively. Clipped away from the exact 0/1 edges
// since logit(0) and logit(1) are +/-Infinity.
export function clipProb(p, lo = 0.02, hi = 0.98) { return Math.min(hi, Math.max(lo, p)); }
export function logit(p) { const c = clipProb(p); return Math.log(c / (1 - c)); }
