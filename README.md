...
Scoped to DraftKings and theScore Bet only (so every parlay stays placeable as one slip on one book).
...
- **Form, usage, venue, weather-historical, birthday** — season/last-3/vs-opponent hit rate, snap%/target
  share/aDOT, dome-vs-outdoor + venue-specific splits, wet/dry historical splits, proximity to a player's
  birthday — all from the player's own game logs.
...
|---|---|---|
| SportsGameOdds (Rookie tier) | DraftKings + theScore Bet event/prop odds | $99/month |
...
about every 3 minutes. The shipped schedule (`.github/workflows/refresh.yml`, `schedule: "*/30 * * * *"` —
every 30 minutes) is sized for that budget with real headroom to spare, though object cost scales with events
× markets per call in a way that can't be verified without watching real usage — check the SportsGameOdds
dashboard after the first week and back the cron off (e.g. hourly) if usage runs hotter than expected.
`lib/fetchers/odds.js` still retries a 429 a few times with backoff and logs rather than crashing on failure,
which costs nothing to leave in place even though it should be rare at this tier.
...
category actually resolves at least once (not just that the code runs without throwing) — including the
defense-vs-position and EPA-matchup-edge factors — and that every game line is a Total (never a Moneyline or
Spread row).

## Data-quality gate

Comparing only two books for a genuine sharp mispricing rarely turns up an edge bigger than a couple of
points. If a row's edge exceeds 8%, it's flagged `suspect` — almost certainly a side/price mismatch somewhere
in the feed rather than a real price — and excluded from Mispriced Bets, AI commentary, and parlay legs. It
still shows in Game Lines/Player Props with a struck-through red "⚠ unverified" badge so you can review the
raw fields (also logged) rather than the row just silently disappearing.
...
lib/
  ...
  store.js         Netlify Blobs wrapper (snapshot, notes, injury history, price history) — works both from
