...
`trueEdge`, or the reverse; the Edge Board, Mispriced Bets, and parlay builder now all rank on `trueEdge`, gated
by `confidence` (`low`/`medium`/`high`/`excluded`) so a thin sample with a lucky-looking edge number can't
outrank a well-supported one. Every card in the UI shows both the model/market probabilities and which
confidence tier backs them, plus the specific factors that moved the number (`row.modelContributors`).
...
found live to 400 the whole request at this tier and was removed — see "Odds-fetch resilience" above). The
shipped schedule (`.github/workflows/refresh.yml`,
...
probability model or a factor's shape changes, confirmed live right after this session's rebuild) means every
row needs a fresh note in the same refresh. Batches of rows within one annotation pass (game lines, props) now
run several at a time (`CONCURRENCY = 4` in `lib/ai.js`) instead of strictly one after another — a live cold-cache
refresh ran past 13 minutes before this fix, almost entirely spent waiting on sequential Anthropic round-trips.
The concurrency cap is deliberate, not laziness: fully parallel would trade a slow refresh for a burst of 429s
...
lib/
  ...
  parlays.js       risk-tiered parlay builder (now ranks legs on the probability model, not point bonuses)
  ...
  store.js         Netlify Blobs wrapper (snapshot, notes, injury/price history, AI note cache, weekly picks,
