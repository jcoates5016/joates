...
`lib/probability.js` turns all of this into `modelProb`/`marketProb`/`edge`/`confidence` per bet, how
`scripts/backtest.js` checks each contextual factor against real history, and how the live results ledger grades
real picks against real outcomes over time.
...
2. **Apply small, named contextual nudges** (favorable matchup, high snap share, heavy red-zone share, opponent
   ...
   instead of double-counting each other. Each nudge's size lives in `lib/modelCoeffs.js`.
...
The result is `{ modelProb, marketProb, edge, confidence }` on every prop and game-line row. **`edge` (exposed as
...
shows both the model/market probabilities and which confidence tier backs them, plus the specific factors that
moved the number (`row.modelContributors`).
...
earning their name, and that will show up here rather than staying a permanent unknown. Game-line (Totals) picks
are saved but not yet graded — that needs each game's final combined score, which isn't fetched anywhere in the
pipeline yet.
...
`lib/parlays.js` builds three kinds of parlay, all sharing the same tier structure and the same leg-selection
rule: **Low Risk = 3 legs, Medium = 4, High = 5, Mega = 6, and every tier always picks the legs the model rates
most likely to hit** (real `modelProb`, gated at a 55% floor — a coin-flip or worse doesn't belong in a build
whose whole premise is "the safest plays on the board"), never the biggest edge/value. This replaced an earlier
edge/conviction-driven system (Low = favorites with real backing, Mega = real longshots allowed, at Jon's
explicit request) — Mega no longer means "longshot payout hunt," it means "6 of the board's best favorites
stacked together." More legs still means a bigger payout and a lower combined probability (that's just how
parlay math works), so the tiers remain a real risk gradient.

- **Risk Tiers** (cross-game) — the original parlay type: pools legs from every game on the board, capped at 2
  legs from any single game so a "board-wide" parlay can't quietly turn into one team's SGP.
- **Same Game Parlays** — one Low/Medium/High/Mega set per game, built only from that game's own legs. No
  contradiction guard is needed: this app only ever surfaces the "over"/"yes" side of every market (props
  exclude under/no, game lines are Totals-only), so there's no opposite-side pairing possible within one game to
  guard against.
- **Slate parlays** — one Low/Medium/High/Mega set per Sunday kickoff window (the "1:00 PM ET Slate" and "4:00
  ...
  cross-game cap since a slate is already scoped to a handful of games, while still requiring at least 2
  different games to fill a 6-leg Mega parlay. A game's window is classified by its real kickoff hour converted
  to Eastern time (`classifyKickoffWindow`), not a hardcoded UTC offset — a fixed offset would silently drift by
  an hour after the November daylight-saving change, right in the middle of a season. Thursday, Sunday night,
  Monday, and early international Sunday kickoffs sit outside both windows and only ever get a Same Game Parlay.
...
means every row needs a fresh note in the same refresh. Batches of rows within one annotation pass (game lines,
props) run several at a time (`CONCURRENCY = 4` in `lib/ai.js`) instead of strictly one after another — a live
cold-cache refresh ran past 13 minutes before this fix, almost entirely spent waiting on sequential Anthropic
round-trips. The concurrency cap is deliberate, not laziness: fully parallel would trade a slow refresh for a
burst of 429s against Anthropic's own per-minute rate limit.
...
- **Cheaper model.** `ANTHROPIC_MODEL` now defaults to `claude-haiku-4-5-20251001` instead of a Sonnet snapshot.
  Every one of these calls (game-line notes, prop notes, scouting takes, parlay rationale) is a "cite the real
  numbers you're given in a sentence or two" task, never deep reasoning, so Haiku's quality is indistinguishable
  here for a fraction of the per-token cost.
- **Only the top 50 rows get a note at all.** `selectAiEligible(gameLines, propRows, limit)` (called once in
  `lib/pipeline.js` right after every row's `modelProb` is computed) ranks the combined pool of non-suspect,
  non-mismatched, model-scored rows by real `modelProb` descending — i.e. genuinely "most likely to hit," not
  best edge/value — and marks the top `AI_NOTE_LIMIT` (50 by default) rows with `_aiSelected = true`.
  `annotateGameLinesWithAI` and `annotatePropsWithAI` only consider rows carrying that flag, so the other rows on
  the board simply don't get an AI note (they still get every non-AI factor and the model's own probability/edge
  numbers — nothing else about them is degraded). Raise or lower the cap by passing a different `limit` at that
  call site.
- **Looser cache matching.** `roundForHash(content)` rounds every number in a row's content before it's hashed
  ...
  miss): values within ±1.5 (rates, probabilities, edges) round to the nearest 0.02, values up to ±20 (point
  spreads, wind speed, small totals) round to the nearest 2, everything larger (prices, yardage, bigger totals)
  rounds to the nearest 5. A book's price ticking by a cent or a forecast's wind estimate drifting by a notch no
  longer forces a fresh Anthropic call for a note that would say the same thing anyway.
...
is a Total (never a Moneyline or Spread row). Also checks the probability model directly: modelProb/edge/
confidence are well-formed on every scored row, a player-out override collapses the estimate near zero
regardless of other factors, a thin 2-game sample stays close to the market's own number while a real deep trend
moves further, Mispriced Bets is actually sorted by real edge, and the results ledger (`lib/grading.js`) grades a
synthetic hit and miss correctly and never double-grades an already-graded pick. Also checks the roster/
depth-chart accuracy pass directly: the roster index resolves a traded player to his latest week's team (not
raw file order), the depth-chart index resolves real ranks/group sizes, `resolvePlayer` prefers the fresher
depth-chart team and flags a genuine conflict, and `findKeyTeammate` picks the depth chart's real next-ranked
player (while still never doing so for a QB).
...
lib/
  ...
  grading.js       results ledger: grades completed picks, folds them into the all-time calibration ledger
  parlays.js       fixed-leg-count (3/4/5/6), probability-first parlay builder — cross-game, Same Game
                   Parlays, and the two Sunday slate windows (see "Parlays" above)
