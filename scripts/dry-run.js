...
import { annotateGameLinesWithAI, annotateScoutingTakes, selectAiEligible, roundForHash } from "../lib/ai.js";
import { classifyKickoffWindow, summarizeEvents, buildSameGameParlays, buildSlateParlays } from "../lib/parlays.js";
...
// controls test below) — annotateGameLinesWithAI only considers rows already marked this way.
const syntheticGameLines = Array.from({ length: 25 }, (_, i) => ({
  oddID: `gl-${i}`, matchup: "Demo Away @ Demo Home", market: "Total", side: "over 47.5",
  bestEdge: 0.05, bestPrice: -110, bestBook: "draftkings", prices: {}, suspect: false, factors: {}, _aiSelected: true
}));
...
// spend problem. selectAiEligible must pick only the real top AI_NOTE_LIMIT rows by modelProb ("most likely to
// hit"), combined across game lines and props, and must still exclude a suspect/mismatched/unscored row even
// if its raw modelProb would otherwise put it in the top slice.
...
const manyLines = [
  { oddID: "gl-high", suspect: false, teamMismatch: false, model: { available: true }, modelProb: 0.9 }, // should always make the cut
  { oddID: "gl-suspect", suspect: true, teamMismatch: false, model: { available: true }, modelProb: 0.85 }, // high prob but suspect -> excluded
  { oddID: "gl-unscored", suspect: false, teamMismatch: false, model: { available: false }, modelProb: null } // model never resolved -> excluded
];
const selected = selectAiEligible(manyLines, manyProps, 50);
const selectedIds = new Set(selected.map(r => r.oddID));
// gl-high takes 1 of the 50 slots, leaving room for the top 49 props by modelProb (i=59 down to i=11).
const top49PropIds = new Set(manyProps.slice(11, 60).map(r => r.oddID));
const aiSelectionWorks = selected.length === 50 && selectedIds.has("gl-high") &&
  !selectedIds.has("gl-suspect") && !selectedIds.has("gl-unscored") &&
  [...top49PropIds].every(id => selectedIds.has(id)) && !selectedIds.has("p-0") && !selectedIds.has("p-10") &&
  manyProps.filter(r => r._aiSelected).length === 49; // 50 total minus gl-high
console.log("selectAiEligible keeps only the top AI_NOTE_LIMIT rows by real modelProb, across lines+props together (should be true):", aiSelectionWorks, `selected=${selected.length}`);
...
// --- Same Game / Slate parlays (lib/parlays.js) ---
...
const demoEvents = summarizeEvents(snapshot.gameLines, snapshot.propRows);
const eventWindows = Object.fromEntries(demoEvents.map(e => [e.eventId, e.window]));
const demoEventWindowsAreCorrect = eventWindows["demo-1"] === "sun_early" && eventWindows["demo-2"] === "sun_early" &&
...
// Regression guard for the fixed-leg-count tier rework: a Same Game Parlay is scoped to ONE game's own legs, so
// demo-1 (which has 6 legs clearing the probability floor once Josh Allen's props are added — see demoData.js)
// must be able to fill every tier up to Mega (6 legs), always picking the highest real-probability legs
// available, never dipping below the probability floor to hit a leg count.
const sameGameParlays = buildSameGameParlays(snapshot.gameLines, snapshot.propRows);
const demo1Sgp = sameGameParlays.find(g => g.eventId === "demo-1");
const sgpFillsEveryTier = demo1Sgp && [3, 4, 5, 6].every((n, i) => demo1Sgp.tiers[i].ok && demo1Sgp.tiers[i].legs.length === n);
const sgpLegsAllFromOneGame = demo1Sgp && demo1Sgp.tiers.every(t => !t.ok || t.legs.every(l => l.gameKey === "demo-1"));
const sgpLegsAreDescendingProbability = demo1Sgp && demo1Sgp.tiers[3].ok &&
  demo1Sgp.tiers[3].legs.every((l, i) => i === 0 || demo1Sgp.tiers[3].legs[i - 1].hitProbability >= l.hitProbability);
console.log("Same Game Parlay fills every tier (3/4/5/6 legs) from one game's own legs (should be true):", sgpFillsEveryTier, demo1Sgp?.tiers.map(t => t.ok ? t.legs.length : t.reason));
console.log("Every Same Game Parlay leg actually comes from that one game (should be true):", sgpLegsAllFromOneGame);
console.log("Same Game Parlay legs are the highest real probability available, in order (should be true):", sgpLegsAreDescendingProbability);

// A game with too few qualifying legs of its own (demo-2 only clears 3) must report exactly which tiers it
// can't build, rather than silently padding out weak legs (or a different game's legs) to hit the count.
const demo2Sgp = sameGameParlays.find(g => g.eventId === "demo-2");
const sgpReportsShortfallHonestly = demo2Sgp?.tiers[0].ok && demo2Sgp.tiers[0].legs.length === 3 &&
  [1, 2, 3].every(i => demo2Sgp.tiers[i].ok === false);
console.log("A game with only 3 qualifying legs builds Low Risk but honestly fails Medium/High/Mega (should be true):", sgpReportsShortfallHonestly, demo2Sgp?.tiers.map(t => t.ok ? t.legs.length : "fail"));

// Regression guard for slate pooling: the 1:00 and 4:00 windows must each see exactly their own 2 demo games
// (never demo-5, the Thursday game) and must be able to fill Mega (6 legs) by drawing from BOTH games in the
// window, not just one — proving the pool is real cross-game pooling, not accidentally scoped to a single event.
const slateParlays = buildSlateParlays(snapshot.gameLines, snapshot.propRows);
const earlySlate = slateParlays.find(s => s.window === "sun_early");
const lateSlate = slateParlays.find(s => s.window === "sun_late");
const slateGameCountsAreCorrect = earlySlate?.games === 2 && lateSlate?.games === 2;
const slateFillsMegaAcrossGames = earlySlate?.tiers[3].ok && new Set(earlySlate.tiers[3].legs.map(l => l.gameKey)).size >= 2 &&
  lateSlate?.tiers[3].ok && new Set(lateSlate.tiers[3].legs.map(l => l.gameKey)).size >= 2;
const thursdayGameNeverJoinsASlate = slateParlays.every(s => s.tiers.every(t => !t.ok || t.legs.every(l => l.gameKey !== "demo-5")));
console.log("Each Sunday slate window sees exactly its own 2 games (should be true):", slateGameCountsAreCorrect, `early=${earlySlate?.games} late=${lateSlate?.games}`);
console.log("A slate Mega parlay draws legs from more than one game in that window (should be true):", slateFillsMegaAcrossGames);
...
if (anyMismatch || !anyRealFactor || missing.length || !anyRedZone || !anyDefense || !anyMatchupEdge || !anyGameLineMatchupEdge || !anyScoringEnv || anyGameLineIsNotTotal || !anyAnytimeTd || !anytimeTdKeptOnlyYesNo || !mahomesTdHitRateIsZero || !recYdsRateIsCorrect || !last10ShapeIsCorrect || !last3IsCurrentSeasonOnly || !rate10IsCorrect || !mahomesTendencyIsSkipped || !anyParlayHasAlternates || !widerBookCoverageWorks || !secondaryInjuryWorks || !venueSplitIsRealStat ||
  ...
  !aiSelectionWorks || !cacheLoosening || !scoutingThrottleWorks ||
  !dstSafetyWorks || !demoEventWindowsAreCorrect || !sgpFillsEveryTier || !sgpLegsAllFromOneGame || !sgpLegsAreDescendingProbability || !sgpReportsShortfallHonestly ||
  !slateGameCountsAreCorrect || !slateFillsMegaAcrossGames || !thursdayGameNeverJoinsASlate) {
  console.log("\nFAILED — see above.");
  process.exit(1);
} else {
